...
function Home() {
  useEffect(() => {
    // Sélectionnez tous les liens ancre internes (ancres pointant vers des sections sur la même page)
      ...
      link.addEventListener("click", (event) => {
        ...
        if (targetElement) {
          gsap.to(window, {
            ...
            scrollTo: { y: targetElement.offsetTop }, // Faites défiler en douceur jusqu'à l'élément cible
            ease: "power2.out", // Courbe d'animation (facultatif, vous pouvez choisir celle qui vous convient)
          });
        }
      });
    });
